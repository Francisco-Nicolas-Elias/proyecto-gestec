...
Define el comportamiento del sistema para el auto-registro de nuevos usuarios institucionales mediante email `@ies21.edu.ar` (validación de dominio, registro pendiente, verificación por email, activación de cuenta), y el mecanismo de sesión: el JWT viaja en una cookie `httpOnly` en vez de `localStorage`, para no ser accesible desde JavaScript del lado del cliente.
